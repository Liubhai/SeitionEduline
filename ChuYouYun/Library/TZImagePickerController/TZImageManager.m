...
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo completion:(void (^)(TZAlbumModel *))completion{
    ...
    if (iOS8Later) {
        ...
        for (PHAssetCollection *collection in smartAlbums) {
            //坑逼三方
            if ([collection.localizedTitle isEqualToString:@"Camera Roll"] || [collection.localizedTitle isEqualToString:@"All Photos"] || [collection.localizedTitle isEqualToString:@"相机胶卷"]|| [collection.localizedTitle isEqualToString:@"所有照片"]) {
        ...
        [self.assetLibrary enumerateGroupsWithTypes:ALAssetsGroupAll usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
            ...
            if ([name isEqualToString:@"Camera Roll"] || [name isEqualToString:@"相机胶卷"] || [name isEqualToString:@"All Photos"]) {
