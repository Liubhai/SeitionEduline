...
-(void)translation:(NSString * _Nonnull )aTranslationString aTranslationComplete:(bTranslationComplete _Nonnull )aTranslationComplete{
    ...
    aTranslationString = [NSAttributedString tkRemoveEmojiAttributedString:aTranslationString withFont:TEXT_FONT withColor:[UIColor whiteColor]];
   ...
   unichar ch = [aTranslationString characterAtIndex:0];
    ...
    if (![TKEduClassRoom shareInstance].roomJson.configuration.isChineseJapaneseTranslation) {
        ...
        if (chNum == aTranslationString.length) {
            //纯中文，则中译日
            tTo = @"jp";
            tFrom = @"zh";
        }
...
#pragma mark 下课
+(void)classBeginEnd:(NSString * _Nonnull )roomID companyid:(NSString *)companyid  aHost:(NSString*_Nonnull)aHost aPort:(NSString *_Nonnull)aPort userid:(NSString *_Nullable)userid roleid:(NSString *_Nullable)roleid aComplete:(bComplete _Nonnull )aComplete aNetError:(bError _Nullable) aNetError{
    
    [[self shareInstance]classBeginEnd:roomID companyid:companyid  aHost:aHost aPort:aPort userid:userid roleid:roleid aComplete:aComplete aNetError:aNetError];
}
...
-(void)classBeginEnd:(NSString * _Nonnull )roomID companyid:(NSString *)companyid aHost:(NSString*_Nonnull)aHost aPort:(NSString *_Nonnull)aPort userid:(NSString *_Nullable)userid roleid:(NSString *_Nullable)roleid aComplete:(bComplete _Nonnull )aComplete aNetError:(bError _Nullable) aNetError
{
    ...
    tParamDic = [NSMutableDictionary dictionaryWithDictionary:@{@"serial":roomID,@"act":@(3),@"companyid":companyid}];
    if (userid && userid.length) {
        [tParamDic setValue:userid forKey:@"userid"];
    }
...
#pragma mark 上课
+(void)classBeginStar:(NSString * _Nonnull )roomID companyid:(NSString *)companyid  aHost:(NSString*_Nonnull)aHost aPort:(NSString *_Nonnull)aPort userid:(NSString *_Nullable)userid roleid:(NSString *_Nullable)roleid aComplete:(bComplete _Nonnull )aComplete  aNetError:(bError _Nullable) aNetError{
    
    [[self shareInstance]classBeginStar:roomID companyid:companyid  aHost:aHost aPort:aPort userid:userid roleid:roleid aComplete:aComplete aNetError:aNetError];
}

-(void)classBeginStar:(NSString * _Nonnull )roomID companyid:(NSString *)companyid aHost:(NSString*_Nonnull)aHost aPort:(NSString *_Nonnull)aPort userid:(NSString *_Nullable)userid roleid:(NSString *_Nullable)roleid aComplete:(bComplete _Nonnull )aComplete aNetError:(bError _Nullable) aNetError
{
    
    NSMutableDictionary *tParamDic =  [NSMutableDictionary dictionaryWithDictionary:@{@"serial":roomID,@"companyid":companyid}];
    ...
    NSString *url = [NSString stringWithFormat:@"%@://%@:%@/ClientAPI/roomstart",sHttp,aHost ,aPort];
    
    session =   [manager GET:url parameters: tParamDic progress:^(NSProgress * _Nonnull uploadProgress) {
...
//    result=-1表示没有数据
    ...
    __block NSURLSessionDataTask *session = nil;
    NSString *url = [NSString stringWithFormat:@"%@://%@:%@/ClientAPI/getupdateinfo",sHttp,aHost ,aPort];
    session =   [manager GET:url parameters: tParamDic progress:^(NSProgress * _Nonnull uploadProgress) {
...
+ (void)downLoadTaskToSandboxWithHost:(NSString *_Nonnull)host taskDic:(NSDictionary *_Nonnull)taskDic complete:(bComplete _Nonnull )aComplete aNetError:(bError _Nullable) aNetError {
    ...
    NSString *pathDir = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:[NSString stringWithFormat:@"Trophy"]];
...
//获取课件备注
+(void)getRemarkWithRoomID:(NSString *_Nonnull)aRoomId aHost:(NSString*_Nonnull)aHost aPort:(NSString *_Nonnull)aPort fileid:(NSString *)fileid complete:(bComplete _Nonnull )aComplete error:(bError _Nullable) aNetError;
{
    //fileid 课件id
    NSDictionary *tParamDic = @{@"serial":aRoomId,
                                @"fileid":fileid};
    ...
    session = [manager GET:[NSString stringWithFormat:@"%@://%@:%@/webaddress/ClientAPI/getfileremark",sHttp,aHost ,aPort] parameters:tParamDic progress:^(NSProgress * _Nonnull downloadProgress) {
