...
@end
@interface TKEduSessionHandle ()<TKRoomManagerDelegate, TKPlaybackManagerDelegate, TKWhiteBoardManagerDelegate>
...
//@property (nonatomic,strong) NSMutableSet   *iUserPlayAudioArray; //音频用户
...
//        NSString *version = [NSString stringWithFormat:@"%s", TKRoomSDKVersionString];
        _playbackMgr              = [TKPlaybackManager instance];
        [_playbackMgr initWithAppKey:kAppKey optional:@{TKRoomSettingOptionalWhiteBoardNotify:@(YES),
                                                        TKRoomSettingOptionalSecureSocket:isHTTPS,
                                                        TKRoomSettingOptionalReconnectattempts:@(5)
                                                        }];
        _roomMgr      = [TKRoomManager instance];
        
        [_roomMgr initWithAppKey:kAppKey optional:@{TKRoomSettingOptionalWhiteBoardNotify:@(YES),
                                                    TKRoomSettingOptionalSecureSocket:isHTTPS,
                                                    TKRoomSettingOptionalReconnectattempts:@(5)
                                                    }];
...
//        bool isConform = [TKUtil  deviceisConform];
    ...
    if (self.isPlayback) {
        [_playbackMgr joinPlaybackRoomWithHost:tHost port:(int)[tPort integerValue] nickName:tNickName roomParams:aParamDic userParams:aProperties];
    } else {
        [_roomMgr joinRoomWithHost:sHost port:(int)[tPort integerValue] nickName:tNickName roomParams:aParamDic userParams:aProperties];
...
- (int)sessionHandlePlayAudio:(NSString *)peerID
    ...
    return [_roomMgr playAudio:peerID completion:^(NSError *error) {
...
- (void)sessionHandleChangeUserPublish:(NSString*)peerID Publish:(int)publish completion:(void (^ _Nullable)(NSError *error))block {
    ...
    [_roomMgr changeUserProperty:peerID tellWhom:@"__all" key:@"publishstate" value:@(publish) completion:block];
}
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    [_roomMgr enableLocalVideo:enable];
#pragma clang diagnostic pop
...
- (void)sessionHandleGetUserWithPeerID:(NSString *)peerID complete: (void(^)(TKRoomUser *))complete{
    ...
    if (tkUser) {
        ...
        [_roomMgr getRoomUserWithPeerId:peerID callback:^(TKRoomUser * _Nullable user, NSError * _Nullable error) {
...
/*
 ...
 1. 巡课不可翻页
 2. 老师可以添加 白板页, 可翻页并同步.
 3. 配置项 canPageTurningFlag 学生可翻页, 授权后 配置项不起作用, 学生可翻页并同步.
 4. 配置项 isHiddenPageFlip 禁用翻页控件
 5. 未上课不同步
 */
- (void)wbSessionManagerPrePage
{
    ...
    if ([TKRoomManager instance].localUser.role == TKUserType_Student) {
        if (_eduClassRoom.roomJson.configuration.canPageTurningFlag == NO &&
            [TKRoomManager instance].localUser.canDraw == NO) {
            return;
        }
    }
...
- (void)wbSessionManagerTurnToPage:(int)pageNum
{
    ...
    if ([TKRoomManager instance].localUser.role == TKUserType_Student) {
        ...
        if (_eduClassRoom.roomJson.configuration.canPageTurningFlag == NO && [TKRoomManager instance].localUser.canDraw == NO) {
            return;
        }
    }
...
- (void)sessionHandleFullScreenSend:(BOOL)isFull
{
    ...
    NSString * josnStr = [TKUtil dictionaryToJSONString:@{@"fullScreenType":@"courseware_file", sNeedPictureInPictureSmall: @YES}];
    
    if (isFull) {
        
        [[TKEduSessionHandle shareInstance] sessionHandlePubMsg:sWBFullScreen ID:sWBFullScreen
                                                             To:sTellAll Data:josnStr Save:true
                                                AssociatedMsgID:nil AssociatedUserID:nil
...
- (void)wbSessionManagerDidSelectDrawType:(int)type color:(NSString *)hexColor widthProgress:(float)progress
{

    [[TKWhiteBoardManager shareInstance] didSelectDrawType:type color:hexColor widthProgress:progress];
}
...
- (void)roomManagerOnUserVideoStatus:(NSString *)peerID
    ...
    if (!self.UIDidAppear) {
        NSString *methodName = NSStringFromSelector(@selector(sessionManagerVideoStateWithUserID:publishState:));
    ...
    if (_iSessionDelegate && [_iSessionDelegate respondsToSelector:@selector(sessionManagerVideoStateWithUserID:publishState:)]) {
        [(id<TKEduSessionDelegate>)_iSessionDelegate sessionManagerVideoStateWithUserID:peerID publishState:state];
...
- (void)roomManagerOnUserAudioStatus:(NSString *)peerID
                               state:(TKMediaState)state {
    if (!self.UIDidAppear) {
        NSString *methodName = NSStringFromSelector(@selector(sessionManagerAudioStateWithUserID:publishState:));
    ...
    if (_iSessionDelegate && [_iSessionDelegate respondsToSelector:@selector(sessionManagerAudioStateWithUserID:publishState:)]) {
        [(id<TKEduSessionDelegate>)_iSessionDelegate sessionManagerAudioStateWithUserID:peerID publishState:state];
...
- (void)roomManagerOnAudioStatsReport:(NSString *)peerId stats:(TKAudioStats *)stats {
    ...
    if (_iSessionDelegate && [_iSessionDelegate respondsToSelector:@selector(sessionManagerOnAVStateWithPeerID:state:)]) {
        [(id<TKEduSessionDelegate>)_iSessionDelegate sessionManagerOnAVStateWithPeerID:peerId state:stats];
    }
}
...
- (void)roomManagerOnConnectionLost{
    ...
    if (_iSessionDelegate && [_iSessionDelegate respondsToSelector:@selector(sessionManagerOnConnectionLost)]) {
...
- (void)roomManagerOnShareScreenState:(NSString *)peerId state:(TKMediaState)state {
    ...
    if (!self.UIDidAppear) {
        NSString *methodName = NSStringFromSelector(@selector(roomManagerOnShareScreenState:state:));
    ...
    if (_iSessionDelegate && [_iSessionDelegate respondsToSelector:@selector(sessionManagerOnShareScreenState:state:)]) {
        
        [(id<TKEduSessionDelegate>) _iSessionDelegate sessionManagerOnShareScreenState:peerId state:state ];
    }
}
...
- (void)onWhiteBoardOnRoomConnectedMsglist:(NSDictionary *)msgList
{
    ...
    for (NSDictionary *tParamDictemp in sorted) {
        ...
        // change: msglist 中如果有showpage 就不发changeDocument消息 教室连接成功的消息列表中没有 showpage 导致课件不同步或加载不出
...
- (void)onWhiteBroadPubMsgWithMsgID:(NSString *)msgID msgName:(NSString *)msgName data:(NSObject *)data fromID:(NSString *)fromID inList:(BOOL)inlist ts:(long)ts
{
    ...
    // 激光笔
    if ([tDataDic[@"eventType"] isEqualToString:@"laserMarkEvent"]) {
        return;
    }
    // 关联课件
    if ([msgName isEqualToString:sDocumentChange])
    {
        if (_eduClassRoom.roomJson.configuration.associatedCourseware == NO) {// 关联课件 免刷新
            return;
        }
        
        // 删除
        if ([tDataDic objectForKey:@"isDel"] && [[tDataDic objectForKey:@"isDel"] boolValue]) {
            ...
            if ([[tDataDic objectForKey:@"isMedia"] boolValue]) {
                ...
                TKUserRoleType role = self.localUser.role;
                
                BOOL isCurrntDM = [self isEqualFileId:tDocmentDocModel aSecondModel:_iCurrentDocmentModel];
                
                //(学生-当前文档-未上课，删除时显示白板
                if ((role == TKUserType_Student) && isCurrntDM &&!_isClassBegin) {
...
//                    [self.whiteBoardManager changeDocumentWithFileID:model.fileid isBeginClass:self.isClassBegin isPubMsg:YES];
                ...
                //老师，巡课-当前文档
                if ((role == TKUserType_Teacher||role == TKUserType_Patrol)&& isCurrntDM) {
                    if (!_isClassBegin) {
//                        [self.whiteBoardManager changeDocumentWithFileID:[self getNextDocment:tDocmentDocModel].fileid isBeginClass:self.isClassBegin isPubMsg:YES];
            ...
            // 删除课件
            [_whiteBoardManager delDocumentFile:tDataDic];
        }
        else {
            ...
            [_whiteBoardManager addDocumentWithFile:tDataDic];
        }
    }
    else if([msgName isEqualToString:sWBPageCount]|| [msgName isEqualToString:sShowPage] ||[msgName isEqualToString:sSharpsChange] )
    {
...
- (void)onWhiteBoardExitAnnotation{
    ...
    if (self.iWhiteBoardDelegate && [self.iWhiteBoardDelegate respondsToSelector:@selector(boardVideoFlagExit)]) {
        [self.iWhiteBoardDelegate boardVideoFlagExit];
    }
}
...
- (void)onWhiteBoardFullScreen:(BOOL)isFull {
    ...
    if ([TKEduSessionHandle shareInstance].isClassBegin &&
        _eduClassRoom.roomJson.configuration.coursewareFullSynchronize &&
        [TKEduSessionHandle shareInstance].roomMgr.localUser.role == TKUserType_Teacher ) {
        
        [self sessionHandleFullScreenSend:isFull];
    } else {
...
- (BOOL)hasPublishUser:(TKRoomUser *)aRoomUser {
    if([[_iPublishDic allKeys] containsObject:aRoomUser.peerID])
    {
        return YES;
    ...
    if (!isUserExist) {
        ...
        if (_eduClassRoom.roomJson.configuration.sortSmallVideo) {
            ...
            if(aRoomUser.role != TKUserType_Patrol){
                NSInteger i = [TKSortTool sortByNameWithArray:_iUserStdAndTchrList peerID:aRoomUser.peerID tExist:isTeacherExist];
            ...
            if ([aRoomUser.peerID isEqualToString: self.localUser.peerID] && aRoomUser.role != TKUserType_Patrol){
...
//            break;
    ...
    for (NSString *tPeer in tDic) {
        ...
        if (tUser.role != TKUserType_Patrol) {
...
- (NSArray *)classDocmentArray
{
    ...
    for (TKDocmentDocModel *model in docArray) {
        if ([model.filecategory integerValue] == 0) {
...
//    }
    ...
    for (TKDocmentDocModel *model in docArray) {
        ...
        if (model.filecategory.integerValue == 1) {
...
//            return [tArray objectAtIndex:tIndex];
    ...
    _iCurrentDocmentModel = [tArray objectAtIndex:0];
    [[TKWhiteBoardManager shareInstance] setCurrentFileId:_iCurrentDocmentModel.fileid];
...
- (BOOL)isEqualFileId:(id)aModel  aSecondModel:(id)aSecondModel
{
    ...
    else if ([aModel isKindOfClass:[TKMediaDocModel class]] && [aSecondModel isKindOfClass:[TKMediaDocModel class]]) {
...
- (NSDictionary *)fileDataDocChangeDic:(TKDocmentDocModel *)aDefaultDocment isDel:(BOOL)isDel
{
    ...
    BOOL isDynamicPPT  = ([tFileProp isEqualToString:@"1"] ||[tFileProp isEqualToString:@"2"] )? true : false ;
...
- (void)addDocMentDocModel:(TKDocmentDocModel*)aDocmentDocModel To:(NSString *)to
{
    ...
    //安卓的文龙说 save 改为false
    [self sessionHandlePubMsg:sDocumentChange ID:sDocumentChange To:to Data:jsonString Save:false AssociatedMsgID:nil AssociatedUserID:nil expires:0 completion:nil];
}
//todo
...
//    if (isHaveAudio) {
//        [self sessionHandleEnableAudio:!aIsPlay];
//    }
...
- (void)startPlayAudioFile:(NSString *)filePath loop:(BOOL)loop {
    if (!_iIsPlaying) {
        [_roomMgr startPlayAudioFile:filePath loop:loop progress:nil];
        _iIsPlaying = YES;
    }
