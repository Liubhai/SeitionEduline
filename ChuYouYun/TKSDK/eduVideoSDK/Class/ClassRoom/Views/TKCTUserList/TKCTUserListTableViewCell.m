...
- (IBAction)removeClick:(UIButton *)sender {
    ...
    TKAlertView *alert = [[TKAlertView alloc]initForWarningWithTitle:MTLocalized(@"Prompt.prompt") contentText:MTLocalized(@"Prompt.KickOutStudent") leftTitle:MTLocalized(@"Prompt.Cancel") rightTitle:MTLocalized(@"Prompt.OK")];
    [alert show];
    alert.rightBlock = ^{
...
- (IBAction)underPlatform:(UIButton *)sender {
    ...
    if (idTime && [idTime isKindOfClass:NSDate.class]) {
        ...
        if (delta < 0.8 && [TKEduSessionHandle shareInstance].onPlatformClickTimes >= 10) {
            [TKUtil showMessage:MTLocalized(@"Prompt.underPlatformTime")];
