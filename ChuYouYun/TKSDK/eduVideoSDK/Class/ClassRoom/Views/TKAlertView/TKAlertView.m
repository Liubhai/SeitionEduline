...
- (id)initForWarningWithTitle:(NSString *)title
    ...
    if (self) {
        ...
        self.leftBtn.sakura.backgroundImage(ThemeKP(@"ok_button_click"),UIControlStateNormal);
        self.rightBtn.sakura.backgroundImage(ThemeKP(@"warn_button_click"),UIControlStateNormal);
    }
    return self;
}
...
- (id)initWithInputTitle:(NSString *)title
    ...
    if (self = [super init]) {
        ...
        if (style == TKErrorCode_CheckRoom_NeedPassword) {
            ...
            // cartoon模版。 提示文字改变
            if (userrole == TKUserType_Teacher) self.alertContentLabel.text = MTLocalized(@"Error.NeedPwd.teacher");
            else if (userrole == TKUserType_Student) self.alertContentLabel.text = MTLocalized(@"Error.NeedPwd.student");
            else if (userrole == TKUserType_Patrol) self.alertContentLabel.text = MTLocalized(@"Error.NeedPwd.tourCourse");
            else self.alertContentLabel.text = MTLocalized(@"Error.NeedPwd");
        ...
        self.inputTextField.placeholder = MTLocalized(@"Prompt.inputPlaceholder");
        self.inputTextField.sakura.textColor(ThemeKP(@"alertInputTextColor"));
        [self.inputTextField setValue:[TKTheme colorWithPath:ThemeKP(@"alertInputPlaceholderColor")] forKeyPath:@"_placeholderLabel.textColor"];
