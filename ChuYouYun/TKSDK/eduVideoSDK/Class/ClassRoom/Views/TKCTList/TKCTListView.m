...
- (id)initWithFrame:(CGRect)frame andTitle:(NSString *)title from:(NSString *)from
{
    if (self = [super initWithFrame:frame]) {
        ...
        //标题空间
        self.titleText = MTLocalized(@"Title.DocumentList");
        ...
        _coursewareListButton = ({
            ...
            [button setTitle:[NSString stringWithFormat:@" %@", MTLocalized(@"Title.DocumentList")] forState:(UIControlStateNormal)];
            [button setTitle:[NSString stringWithFormat:@" %@", MTLocalized(@"Title.DocumentList")] forState:(UIControlStateSelected)];
        ...
        _mediaListButton = ({
            ...
            button.sakura.image(ThemeKP(@"selector_point_default"), UIControlStateNormal);
            button.sakura.image(ThemeKP(@"selector_point_select"),UIControlStateSelected);
            [button setTitle:[NSString stringWithFormat:@"%@ ", MTLocalized(@"Title.MediaList")] forState:(UIControlStateNormal)];
            [button setTitle:[NSString stringWithFormat:@"%@ ", MTLocalized(@"Title.MediaList")] forState:(UIControlStateSelected)];
            button.titleLabel.textAlignment = NSTextAlignmentCenter;
            button.titleLabel.font = [UIFont systemFontOfSize: 12.0];
            button.sakura.backgroundImage(ThemeKP(@"selector_bg_default"),UIControlStateNormal);
            button.sakura.backgroundImage(ThemeKP(@"selector_bg_select"),UIControlStateSelected);
