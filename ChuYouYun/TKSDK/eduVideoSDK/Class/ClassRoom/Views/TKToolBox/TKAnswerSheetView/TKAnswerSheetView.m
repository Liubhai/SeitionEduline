...
- (void)commonInit
{
    self.titleLabel.text = MTLocalized(@"tool.datiqiqi");
...
- (void)setDict:(NSDictionary *)dict
{
    ...
    if (self.viewType == TKAnswerSheetType_Submit) {
        ...
        if (role == TKUserType_Student) {
            ...
            NSString *timeString = [NSString stringWithFormat:@"%@：%@",MTLocalized(@"tool.time"),[self timeStringWithTime:time]];
...
- (void)timerAction:(NSTimer *)timer
{
    ...
    NSString *timeString = [NSString stringWithFormat:@"%@：%02d:%02d:%02d",MTLocalized(@"tool.time"),hours,minutes,seconds];
    self.detailView.timeString = timeString;
    self.recordView.timeString = timeString;
}
