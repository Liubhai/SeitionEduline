...
//        CGFloat viewHeight = fminf(ScreenH / 3, 200);
        ...
        NSURL *imgUrl = [[NSBundle mainBundle] URLForResource:str withExtension:@"gif"];
        NSData * gifImageData = [NSData dataWithContentsOfURL:imgUrl];
//        UIImage * image = [UIImage sd_animatedGIFWithData:gifImageData];
//        gifImage.image = image;
        ...
        sBtn.sakura.backgroundImage(ThemeKP(@"tk_res_surebtn"), UIControlStateSelected);
...
- (void)receiveShowResponderViewWith:(NSDictionary *)dict {
    ...
    if ([TKEduSessionHandle shareInstance].localUser.role == TKUserType_Patrol ||
        ...
        if (_isShow && !_isBegin) {
            self.hidden = NO;
            [self.sureButton setTitle:MTLocalized(@"Res.btn.start") forState:UIControlStateNormal];
            self.tipLabel.text = MTLocalized(@"Res.lab.start");
            
        } else if (_isShow && _isBegin) {
            self.hidden = NO;
            [self.sureButton setTitle:MTLocalized(@"Res.btn.getting") forState:UIControlStateNormal];
            self.tipLabel.text = MTLocalized(@"Res.lab.getting");
            [self performSelector:@selector(allReadyWithObject:) withObject:@(11) afterDelay:8];

        } else if (!_isShow && !_isBegin) {
            self.hidden = NO;
            [self.sureButton setTitle:MTLocalized(@"Res.btn.noget") forState:UIControlStateNormal];
            self.tipLabel.text = MTLocalized(@"Res.lab.noget");
        }
        ...
        if ([TKEduSessionHandle shareInstance].localUser.publishState != TKPublishStateNONE) {
            ...
            if (_isShow && _isBegin) {
                self.hidden = NO;
                [self.sureButton setTitle:MTLocalized(@"Res.readyget") forState:UIControlStateNormal];
                self.tipLabel.text = MTLocalized(@"Res.readyget");
...
- (void) allReadyWithObject:(id)obj {
    ...
    if (type == 1) {
        ...
        [self.sureButton setTitle:MTLocalized(@"Res.lab.noget") forState:UIControlStateNormal];
        self.tipLabel.text = MTLocalized(@"Res.lab.noget");
        self.sureButton.userInteractionEnabled = NO;
...
- (void)receiveResponderUser:(NSDictionary *)dict peerid:(nonnull NSString *)peerid {
    ...
    if ([TKEduSessionHandle shareInstance].localUser.role == TKUserType_Patrol ||
        ...
        self.sureButton.userInteractionEnabled = YES;
        [self.sureButton setTitle:MTLocalized(@"Res.btn.t.get") forState:UIControlStateNormal];
        ...
        [self.sureButton setTitle:MTLocalized(@"Res.btn.get") forState:UIControlStateNormal];
        if ([[TKEduSessionHandle shareInstance].localUser.peerID isEqualToString:peerid]) {
            self.tipLabel.text = MTLocalized(@"Res.lab.iget");
        } else {
            self.tipLabel.text = dict[@"userAdmin"];
        }
        self.sureButton.userInteractionEnabled = NO;
