...
- (void)setupTextSystem
{
    ...
    _linkDetectionTypes = TKLinkDetectionTypeAll;
    
    self.linkBackgroundColor = [UIColor colorWithWhite:0.95 alpha:1.0];
    self.linkColor = [TXSakuraManager tx_colorWithPath: ThemeKP(@"linkTextColor")];
//    self.linkHighlightColor = [UIColor redColor];
    ...
    self.userInteractionEnabled = YES;
    //默认的回调
    self.linkTapHandler = ^(TKLinkType linkType, NSString *string, NSRange range) {
//        TKLog(@"Link Tap Handler");
    };
    self.linkLongPressHandler = ^(TKLinkType linkType, NSString *string, NSRange range){
//        TKLog(@"Link Long Press Handler");
    };
}
...
- (void)setLinkDetectionTypes:(TKLinkDetectionTypes)linkDetectionTypes
{
...
- (void)updateTextStoreWithAttributedString:(NSAttributedString *)attributedString
{
    if (attributedString.length != 0){
        attributedString = [TKLinkLabel sanitizeAttributedString:attributedString];
    }
...
//    paragraph.alignment = self.textAlignment;
    ...
    NSDictionary *attributes = @{
                                    ...
                                    NSForegroundColorAttributeName : self.isWhiteColor ? UIColor.whiteColor : [TKTheme colorWithPath:@"ClassRoom.TKChatViews.chatMessageColor"],
...
- (NSArray *)getRangesForURLs:(NSAttributedString *)text
{
    ...
    for (NSTextCheckingResult *match in matches)
    {
        ...
        //是否包含汉字，如果有汉字需要将汉字截取掉
        for(int i=0; i< [realURL length];i++){
            int a = [realURL characterAtIndex:i];
            if( a > 0x4e00 && a < 0x9fff)
            {
                realURL = [realURL substringToIndex:i];
                matchRange.length = i;
                break;
            }
        }
    ...
    for (NSTextCheckingResult *match in matches)
    {
        ...
        if ([match resultType] == NSTextCheckingTypePhoneNumber){
            [rangesForPhoneNumbers addObject:@{
                                                 @"linkType" : @(TKLinkTypePhoneNumber),
                                                 @"range"    : [NSValue valueWithRange:matchRange],
                                                 @"link"     : matchString
                                              }];
        }
...
//    {
//        NSRange range = [[dictionary objectForKey:@"range"] rangeValue];
//        TKLinkType linkType = [[dictionary objectForKey:@"linkType"] integerValue];
//        if (linkType == TKLinkTypeURL) {
...
- (IBAction)longPressLabel:(UILongPressGestureRecognizer *)recognizer
{
    ...
    if (link) {
        ...
        TKLinkType linkType = (TKLinkType)[[link objectForKey:@"linkType"] intValue];
...
- (void)labelClick:(UITapGestureRecognizer *)recognizer
{
    ...
    if (link) {
        ...
        NSString *linkString = [link objectForKey:@"link"];
        TKLinkType linkType = (TKLinkType)[[link objectForKey:@"linkType"] intValue];
        self.linkTapHandler(linkType, linkString, range);
    } else {
        return;
    }
}
