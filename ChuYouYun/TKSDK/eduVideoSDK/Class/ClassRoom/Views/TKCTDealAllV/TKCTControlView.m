...
- (id)initWithFrame:(CGRect)frame numberOfStuPlat:(NSInteger)numberOfStuPlat {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        // 标题
        self.titleText = MTLocalized(@"Title.ControlList");
...
- (void)loadAllBtn{
    ...
    self.allRewardBtn = ({
        ...
        TKControlButton *button = [[TKControlButton alloc]initWithFrame:CGRectMake(self.allMuteBtn.leftX, self.allMuteBtn.bottomY + marginY, btnWidth, btnHeight) imageName:ThemeKP(@"btn_reward") disableImageName:ThemeKP(@"btn_reward_unclickable") title:MTLocalized(@"Button.Reward")];
        button.selected = NO;
        [self.contentImageView addSubview:button];
    ...
    self.allResetBtn = ({
        ...
        TKControlButton *button = [[TKControlButton alloc]initWithFrame:CGRectMake(self.allRewardBtn.rightX + marginX, self.allRewardBtn.y, btnWidth, btnHeight) imageName:ThemeKP(@"button_restore") disableImageName:ThemeKP(@"button_restore_unclickable") title:MTLocalized(@"Button.Reset")];
        button.selected = NO;
        [self.contentImageView addSubview:button];
...
//    } else {
    ...
    if (![TKEduSessionHandle shareInstance].isunMuteAudio) {
        ...
        for (TKRoomUser *tUser in [[TKEduSessionHandle shareInstance] userStdntAndTchrArray]) {
            ...
            if(tState == TKPublishStateVIDEOONLY){
                tState = TKPublishStateBOTH;
            }
            [[TKEduSessionHandle shareInstance] sessionHandleChangeUserPublish:tUser.peerID Publish:tState completion:nil];
        }
