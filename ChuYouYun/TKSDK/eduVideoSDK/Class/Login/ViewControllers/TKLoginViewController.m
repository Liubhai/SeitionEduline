...
- (void)viewDidLoad {
    ...
    self.backgroundImageView = ({
        ...
        imageView.image = [UIImage imageNamed:@"Login.login_bg"];
...
- (void)loginButtonAction:(id)sender
{
    ...
    if (!self.nicknameView.inputView.text.length
        ...
        //昵称不能为空
        NSString *content =  MTLocalized(@"Prompt.nicknameNotNull");
    ...
    if (!self.roomidView.inputView.text.length
        ...
        //教室号不能为空
        NSString *content =  MTLocalized(@"Prompt.RoomIDNotNull");
        TKAlertView *alert = [[TKAlertView alloc] initWithTitle:MTLocalized(@"Prompt.prompt") contentText:content confirmTitle:MTLocalized(@"Prompt.Know")];
        [alert show];
        return;
    }
...
//    [[TKEduClassRoom shareInstance] joinPlaybackRoomWithParamDic:parameters
//                                                  ViewController:self
//                                                        Delegate:self
//                                                       isFromWeb:NO];
...
- (UIImageView *)logoImageView
{
    if (!_logoImageView) {
        ...
        _logoImageView.image = [UIImage imageNamed:@"tk_login_logo_black"];
...
- (UIButton *)loginButton
{
    if (!_loginButton) {
        ...
        [_loginButton setBackgroundImage:[UIImage imageNamed:@"tk_bt_join_room_bg_balck"] forState:UIControlStateNormal];
        [_loginButton setBackgroundImage:[UIImage imageNamed:@"tk_bt_join_room_bg_balck"] forState:UIControlStateSelected];
        [_loginButton setTitle:MTLocalized(@"Login.EnterRoom") forState:UIControlStateNormal];
...
{
    if (!_roomidView) {
        _roomidView = [[TKLoginInputView alloc] initWithFrame:CGRectZero showText:nil placeholderText:MTLocalized(@"Label.roomPlaceholder") isShow:NO setImageName:@"tk_icon_room_number_black"];
...
{
    if (!_nicknameView) {
        _nicknameView = [[TKLoginInputView alloc] initWithFrame:CGRectZero showText:nil placeholderText:MTLocalized(@"Label.nicknamePlaceholder") isShow:NO setImageName:@"tk_login_nick_name_icon_black"];
        //限制昵称为24个字符
...
{
    if (!_roleView) {
        _roleView = [[TKLoginInputView alloc] initWithFrame:CGRectZero showText:@"学生" placeholderText:nil isShow:YES setImageName:@"tk_login_right_icon_black"];
