            ...
            if ([[dict stringValueForKey:@"is_free"] integerValue] == 1) {
                ...
                if ([[dict stringValueForKey:@"is_buy"] integerValue] == 1) {
                    ...
                    if ([dict[@"course_hour_price"] floatValue] == 0) {
                        ...
                        _freeLabel.text = [NSString stringWithFormat:@"%.2f金豆",[dict[@"course_hour_price"] floatValue]];
//                        if (([dict[@"course_hour_price"] floatValue] * 100 / 100) >= 1) {//有小数
//
//                        } else {//整数
//                            _freeLabel.text = [NSString stringWithFormat:@"%ld金豆",[dict[@"course_hour_price"] integerValue]];
//                        }
            ...
            if ([[_cellDict stringValueForKey:@"is_buy"] integerValue] == 1) {
                ...
                if ([_cellDict[@"course_hour_price"] floatValue] == 0) {
                    ...
                    if (([_cellDict[@"course_hour_price"] floatValue] * 100 / 100) >= 1) {//有小数
                        _freeLabel.text = [NSString stringWithFormat:@"%.02lf金豆",[_cellDict[@"course_hour_price"] floatValue]];
                    } else {//整数
                        _freeLabel.text = [NSString stringWithFormat:@"%ld金豆",[_cellDict[@"course_hour_price"] integerValue]];
                    }
                    _freeLabel.text = [NSString stringWithFormat:@"%.2f金豆",[_cellDict[@"course_hour_price"] floatValue]];
                    _freeLabel.layer.borderColor = [UIColor redColor].CGColor;
                    _freeLabel.textColor = [UIColor redColor];
