...
@property (nonatomic, strong) BRAFURLSessionManager *sessionManager;
...
{
    __weak ZBLM3u8Downloader *weakSelf  =self;
    ...
    downloadInfo.downloadTask = [self.sessionManager downloadTaskWithRequest:request progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if ( weakSelf.isM3u8Type == NO && weakSelf.downloadProgressHandler) {
            weakSelf.downloadProgressHandler((float)downloadProgress.completedUnitCount / (float)downloadProgress.totalUnitCount,[weakSelf.fileDownloadInfos firstObject].m3u8FullRootUrlString );
        }
...
- (void)verifyDownloadCountAndCallbackByDownloadSuccess:(BOOL) isSuccess
{
    ...
    if (isSuccess) {
        if (_downloadProgressHandler) {
            _downloadProgressHandler(successCount / (float)_fileDownloadInfos.count,[_fileDownloadInfos firstObject].m3u8FullRootUrlString );
        }
...
#pragma mark -
- (BRAFURLSessionManager *)sessionManager
{
    if (!_sessionManager) {
        _sessionManager = [[BRAFURLSessionManager alloc]initWithSessionConfiguration:[NSURLSessionConfiguration defaultSessionConfiguration]];
