...
+ (void)analysisWithUrlString:(NSString*)urlStr completaionHandler:(ZBLM3u8AnalysiseCompletaionHandler)completaionHandler
{
    ...
    if ([urlStr rangeOfString:@".m3u8"].location == NSNotFound) { //不是m3u8
        //ttps://eduline.obs.cn-north-1.myhuaweicloud.com:443/5cf345f9f1537.mp4?AWSAccessKeyId=VWGIHLGHHETPK4KGYQEP&Expires=1560520859&Signature=vP7hvRvJCNJeEQ9p3VhoLIf%2B4z4%3D
        NSString *rootUrl = [[urlStr componentsSeparatedByString:@"?"] firstObject];
        NSString *fileKey = [[rootUrl componentsSeparatedByString:@"/"] lastObject];
        ZBLM3u8Info *info = [ZBLM3u8Info new];
        info.keyMethod = [ZBLM3u8Setting notm3u8type];
        NSMutableArray *temp = [NSMutableArray arrayWithCapacity:0];
        ZBLM3u8TsInfo *tsInfo = [ZBLM3u8TsInfo new];
        tsInfo.oriUrlString = urlStr;
        tsInfo.localUrlString = [NSString stringWithFormat:@"%@/%@/%@",
                                [ZBLM3u8Setting commonDirPrefix],
                                [ZBLM3u8Setting uuidWithUrl:rootUrl],
                                [ZBLM3u8Setting tsFileWithIdentify:fileKey]];
        tsInfo.m3u8FullRootUrlString = urlStr;
        [temp addObject:tsInfo];
        info.tsInfos = temp;
        if (completaionHandler) {
            completaionHandler(info,nil);
        }
        //tsInfo.localUrlString
        return;
    }
    
    NSString *oriM3u8String = [NSString stringWithContentsOfFile:[[ZBLM3u8Setting fullCommonDirPrefixWithUrl:urlStr] stringByAppendingPathComponent:[ZBLM3u8Setting m3u8InfoFileName]] encoding:0 error:nil];
...
+ (void)analysisWithOriUrlString:(NSString*)OriUrlString m3u8String:(NSString*)m3u8String completaionHandler:(ZBLM3u8AnalysiseCompletaionHandler)completaionHandler
{
    ...
    NSString *rootUrl = [OriUrlString componentsSeparatedByString:@".m3u8"].firstObject;
    NSString *lastUrl = [[rootUrl componentsSeparatedByString:@"/"] lastObject];
    rootUrl = [rootUrl stringByReplacingOccurrencesOfString:lastUrl withString:@""];
    /*
    ...
    while (tsRange.location != NSNotFound) {
        @autoreleasepool {
            ...
            if ([tsInfo.oriUrlString rangeOfString:@"http"].location == NSNotFound) {
                tsInfo.oriUrlString = [NSString stringWithFormat:@"%@%@",rootUrl,tsInfo.oriUrlString];
            }
            ...
            tsInfo.m3u8FullRootUrlString = OriUrlString;
