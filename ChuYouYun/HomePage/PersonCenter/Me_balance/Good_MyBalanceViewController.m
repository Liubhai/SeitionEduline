...
- (void)addRechargeView {
    ...
    for (int i  = 0 ; i < allNumber + 1 ; i ++) {
        if (i < _netWorkBalanceArray.count) {
            ...
            number1.text = [NSString stringWithFormat:@"%@马币",[[_netWorkBalanceArray objectAtIndex:i] stringValueForKey:@"rechange"]];
            ...
            if ((isAddAilpayView || isAddWxpayView || isAddRechargeCardView) && !_appleButton.selected) {
                ...
                [moneyButton setTitle:@"马币" forState:UIControlStateNormal];
...
- (void)netWorkUserBalanceConfig {//
    ...
    [op setCompletionBlockWithSuccess:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
        ...
        _balanceLabel.text = [NSString stringWithFormat:@"马币%@",[_balanceDict stringValueForKey:@"balance" defaultValue:@"0"]];
...
- (void)netWorkUserRechargeBalance {//
    ...
    if (_realMoney.text.length > 1) {
        ...
        [TKProgressHUD showError:@"充值马币不能为空" toView:self.view];
        return;
    }
