...
- (void)addPayView {
    ...
    NSArray *titleArray = @[@"金豆",@"收入"];
    NSArray *iconArray = @[@"money",@"income"];
    NSArray *informationArray = @[@"(当前金豆0)",@"(当前账户提成为金豆0)"];
    CGFloat topY = 0;
    for (int i = 0 ; i < 2 ; i ++) {
...
- (void)textChange:(NSNotification *)not {
    ...
    if (_textField.text.length > 8) {
        ...
        _textField.text = @"";
        _remark.text = [NSString stringWithFormat:@"需要花费0个金豆"];
        _realMoney.text = @"金豆 0";
        return;
    } else {
        NSString *textStr = [_textField.text substringFromIndex:0];
        CGFloat hh = [textStr floatValue] / [_sple_score_str integerValue];
        _remark.text = [NSString stringWithFormat:@"需要花费%.2f 金豆",hh];
        _realMoney.text = [NSString stringWithFormat:@"%.2f 金豆",hh];
    }
}
    ...
    [op setCompletionBlockWithSuccess:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
        ...
        if ([[dataSource stringValueForKey:@"code"] integerValue] == 1) {
            ...
            if (_componentsArray.count >= 2) {
                ...
                _exchange.text = [NSString stringWithFormat:@"(注:%@金豆=%@积分)",@"1",_componentsArray[1]];
            ...
            for (int i = 0 ; i < array.count ; i ++) {
                ...
                if ([[dict stringValueForKey:@"pay_num"] isEqualToString:@"lcnpay"]) {//余额支付
                    _balanceseInformationLabel.text = [NSString stringWithFormat:@"(当前金豆%@)",[dict stringValueForKey:@"balance" defaultValue:@"0"]];
                } else if ([[dict stringValueForKey:@"pay_num"] isEqualToString:@"spipay"]) {
                    _splitInformationLabel.text = [NSString stringWithFormat:@"(当前金豆%@)",[dict stringValueForKey:@"balance" defaultValue:@"0"]];
                }
            }
