...
- (void)whichPayView {
    ...
    for (int i = 0 ; i < imageArray.count ; i ++) {
        ...
        if ([_scoreStaus integerValue] == 1) {
            if (i == imageArray.count - 1) {
                ...
                payType.text = [NSString stringWithFormat:@"积分 (当前账号积分为马币%@)",[_userAccountDict stringValueForKey:@"score" defaultValue:@"0"]];
    ...
    price.text = [NSString stringWithFormat:@"实付：马币%.2f",Money];
