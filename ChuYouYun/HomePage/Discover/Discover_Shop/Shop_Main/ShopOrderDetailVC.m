...
- (void)makeSubView {
    ...
    _agreeTitle.text = @"我已阅读并同意《匠人网校解锁协议》";
    _agreeTitle.textColor = RGBHex(0x8B8888);
    _agreeTitle.font = SYSTEMFONT(14);
    NSMutableAttributedString *mutal = [[NSMutableAttributedString alloc] initWithString:@"我已阅读并同意《匠人网校解锁协议》"];
...
- (void)submitButtonClick {
    ...
    if (!_agreeButton.selected) {
        [TKProgressHUD showError:@"请先阅读并同意《匠人网校解锁协议》" toView:self.view];
        return;
    }
