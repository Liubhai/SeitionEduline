...
- (UIView *)speedView {
    if (!_speedView) {
        _speedView = [[UIView alloc] initWithFrame:CGRectMake(MainScreenHeight-100, 0, 100, MainScreenWidth)];
        NSArray *speedArray = @[@"1.0X",@"1.25X",@"1.5X",@"2.0X"];
        for (int i = 0; i<speedArray.count; i++) {
            UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake(0, i*MainScreenWidth/4.0, 100, MainScreenWidth/4.0)];
            btn.tag = i;
            [btn setBackgroundColor:[UIColor lightGrayColor]];
            [btn setTitle:speedArray[i] forState:0];
            [btn setTitleColor:BasidColor forState:UIControlStateSelected];
            [btn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            [_speedView addSubview:btn];
            [btn addTarget:self action:@selector(speedButtonClickChangeSpeed:) forControlEvents:UIControlEventTouchUpInside];
        }
    }
    return _speedView;
}
...
- (void)updateVodPlayViewDataWithEvent:(AliyunVodPlayerEvent)event vodPlayer:(AliyunVodPlayer *)vodPlayer{
    ...
    switch (event) {
        ...
        case  AliyunVodPlayerEventFirstFrame:
        {
            ...
            if (_seekTime > 0) {
                [self.aliPlayer seekToTime:_seekTime];
            }
        }
            break;
...
- (void)showSpeedView {
    ...
    for (UIButton *btn in self.speedView.subviews) {
        if (self.aliPlayer.playSpeed == 1.0) {
            btn.selected = btn.tag == 0 ? YES : NO;
        } else if (self.aliPlayer.playSpeed == 1.25) {
            btn.selected = btn.tag == 1 ? YES : NO;
        } else if (self.aliPlayer.playSpeed == 1.5) {
            btn.selected = btn.tag == 2 ? YES : NO;
        } else if (self.aliPlayer.playSpeed == 2.0) {
            btn.selected = btn.tag == 3 ? YES : NO;
        }
    }
}
...
// MARK: - seekToTime(跳转到指定时间节点播放)
- (void)edulineSeekToTime:(NSTimeInterval)seekTime {
    _seekTime = seekTime;
}

- (void)speedButtonClickChangeSpeed:(UIButton *)sender {
    if (sender.tag == 1) {
        [self.aliPlayer setPlaySpeed:1.25];
    } else if (sender.tag == 2) {
        [self.aliPlayer setPlaySpeed:1.5];
    } else if (sender.tag == 3) {
        [self.aliPlayer setPlaySpeed:2.0];
    } else {
        [self.aliPlayer setPlaySpeed:1.0];
    }
