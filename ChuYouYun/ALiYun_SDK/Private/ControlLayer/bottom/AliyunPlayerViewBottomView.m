...
- (UIButton *)speedButton {
    if (!_speedButton) {
        ...
        [_speedButton setTitle:@"倍速" forState:0];
        [_speedButton setTitleColor:[UIColor whiteColor] forState:0];
        _speedButton.titleLabel.font = [UIFont systemFontOfSize:ALYPVBottomViewTextSizeFont];
    }
    return _speedButton;
}
...
//        self.qualityButton.selected = NO;
    ...
    self.speedButton.frame = CGRectMake(_progressView.right, 0, 25, _progressView.height);
    self.speedButton.centerY = _progressView.centerY;
    [self.speedButton addTarget:self action:@selector(showSpeedViewButtonClick:) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)showSpeedViewButtonClick:(UIButton *)sender {
    if (_delegate && [_delegate respondsToSelector:@selector(showSpeedView)]) {
        [_delegate showSpeedView];
    }
}
